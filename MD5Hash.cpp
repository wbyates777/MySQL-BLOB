// code see http://en.wikipedia.org/wiki/MD5
...
void MD5Final ( unsigned char digest[16], MD5_CTX * context, unsigned char* padding);
...
{	
	/* Tony M: Replaced with RTL memcpy */	
...
{
	/* Tony M: Replaced with RTL memset */	
...
void 
MD5Final( unsigned char digest[16], MD5_CTX * context, unsigned char* padding)
{	
...
std::string 
MD5Hash::hash( const std::string& sInputString )		
{
	MD5_CTX context;
	
	unsigned char digest[16];	
	unsigned int len = (unsigned int) sInputString.length();
	
	MD5Init (&context);	
	MD5Update (&context, (unsigned char*)(const char *) sInputString.c_str(), len);	
	...
	for(int i = 0; i < 16; ++i)		
	{		
		std::snprintf( pos, 64, "%x02", digest[i] );		
		pos+= 2;		
	}
